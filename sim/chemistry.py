"""
Chemistry: covalent bond formation/breaking + nuclear fusion.
...
from sim.elements import ELEMENTS, ELEMENTS_LIST, SYMBOL_TO_ID, fusion_product
...
def _build_grid(pos: np.ndarray, cell_size: float) -> dict:
    grid: dict = {}
...
def _neighbors(idx: int, pos: np.ndarray, grid: dict, cell_size: float) -> list[int]:
    cell = tuple((pos[idx] / cell_size).astype(int))
    result = []
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
...
def _form_bonds(world):
    ...
    # Existing bonded pairs (avoid duplicates)
    bonded = {(min(b.i, b.j), max(b.i, b.j)) for b in world.bonds}

    # Cell size from elements ACTUALLY in the simulation (not the full periodic table).
    # Using the global max (e.g. K=203 pm) makes every particle land in the same cell
    # → spatial hash degenerates to O(N²).  Using present bonding elements keeps cells
    # small enough that each particle has only a handful of neighbours.
    ...
    grid = _build_grid(pos, cell_size)
    ...
    for i in range(world.n):
        ...
        if elem_i.max_bonds == 0:
            continue
        if bond_counts[i] >= elem_i.max_bonds:
            continue

        for j in _neighbors(i, pos, grid, cell_size):
            if j <= i:
                continue
            key = (i, j)
            if key in bonded:
            ...
            if elem_j.max_bonds == 0:
                continue
            if bond_counts[j] >= elem_j.max_bonds:
                continue
...
def _fuse(world):
    ...
    This avoids the O(N²) Python spatial-hash loop that dominates when
    particles are clustered.
    """
    if not world.cfg.chemistry.fusion_enabled:
        return
    if world.n < 2:
        return
    ...
    # --- Identify fusable particles -----------------------------------------
    can_fuse = np.array([ELEMENTS_LIST[world.elem_ids[i]].can_fuse for i in range(n)])
    fi_all   = np.where(can_fuse)[0]          # indices into world arrays
    nf       = len(fi_all)
    if nf < 2:
        return

    pos_f  = world.positions[fi_all]           # (nf, 3)
    vel_f  = world.velocities[fi_all]          # (nf, 3)
    mass_f = world.masses[fi_all]              # (nf,)
    ...
    # --- Pairwise distance screening (numpy, upper triangle) ----------------
    diff    = pos_f[:, np.newaxis, :] - pos_f[np.newaxis, :, :]   # (nf, nf, 3)
    dist_sq = np.einsum('ijk,ijk->ij', diff, diff)                 # (nf, nf)
    ...
    # --- KE screening (numpy) -----------------------------------------------
    rel_vel = vel_f[pi] - vel_f[pj]                                # (M, 3)
    mu      = mass_f[pi] * mass_f[pj] / (mass_f[pi] + mass_f[pj]) # (M,)
    rel_ke  = 0.5 * mu * np.einsum('ij,ij->i', rel_vel, rel_vel)   # (M,)
    ...
    # Sort hottest first so most energetic events are preferred when particles
    # would be claimed by multiple events
    hot = hot[np.argsort(-rel_ke[hot])]

    # --- Collect events (Python loop over the FEW hot pairs) ----------------
    used:   set[int] = set()
    ...
    # --- Apply fusions (largest index first to keep swap-removal valid) ------
